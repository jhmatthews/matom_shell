...
import time
from constants import H_OVER_K, H, C, PI
...
def read_pywind_sphere(filename, return_inwind=False):
	# first, simply load the filename 
	d = np.loadtxt(filename, comments="#", dtype = "float", unpack = True)
	...
	#return the transpose for contour plots.
	if return_inwind:
		return z, masked_values, inwind_bool
	else:
		return z, masked_values.T
...
class xband:
	...
	'''
	def __init__(self, nxtot, xj, ave_freq, specmod, alpha, pl_log_w, exp_w, exp_t, fmin, fmax, J_array, t_r, w):
		...
		self.fmax = fmax
		self.J = J_array
		self.t_r = t_r		# this will be the same in each band
		self.w = w
...
def get_xbands(fname, NPLASMA, NBANDS, mode="cyl"):
	...
	if mode == "spherical":
		...
		x1, pnum = read_pywind_sphere( "%s.pnum.dat" % (fname) )

		print np.max(pnum)

		x1, _tr = read_pywind_sphere( "%s.tr.dat" % (fname) )

		x1, _w = read_pywind_sphere( "%s.w.dat" % (fname) )
		...
		x1, y1, _tr = read_pywind_smart( "%s.tr.dat" % (fname) )

		x1, y1, _w = read_pywind_smart( "%s.w.dat" % (fname) )

	for i in range(NBANDS):

		if mode == "spherical":
			x1, _nxtot = read_pywind_sphere( "%s.nxtot00%i.dat" % (fname, i) )
			x2, _xj = read_pywind_sphere( "%s.xj00%i.dat" % (fname, i) )
			x3, _ave_freq, inwind = read_pywind_sphere( "%s.xave_freq00%i.dat" % (fname, i), return_inwind=True )
			x1, _specmod = read_pywind_sphere( "%s.spec_mod_type00%i.dat" % (fname, i) )
			x1, _exp_t = read_pywind_sphere( "%s.exp_temp00%i.dat" % (fname, i) )
			x1, _exp_w = read_pywind_sphere( "%s.exp_w00%i.dat" % (fname, i) )
			x1, _pl_log_w = read_pywind_sphere( "%s.pl_log_w00%i.dat" % (fname, i) )
			x1, _pl_alpha = read_pywind_sphere( "%s.pl_alpha00%i.dat" % (fname, i) )
			x1, _fmax = read_pywind_sphere( "%s.fmax_mod00%i.dat" % (fname, i) )
			x1, _fmin = read_pywind_sphere( "%s.fmin_mod00%i.dat" % (fname, i) )
		...
		xbands[i] = xband ( nxtot, ave_freq, xj, specmod, pl_alpha, pl_log_w, exp_w, exp_t, fmin, fmax, np.zeros(len(nxtot)), tr, w)
...
def plot_cell(xbands, nplasma, lstyle = "-", mode = "pl"):
	...
	for n in range(len(nplasma)):
		...
		if mode == "pl":
			...
			f, j = J_nu(xbands, nplasma[n], (1e13,1e17))
...
def J_nu (xbands, nplasma, (fmin, fmax), nfreqs =10000):

	''' 
	The planck function for frequency nu at temperature T
	tuple controls frequency limits
	'''

	nu = np.linspace(fmin, fmax, nfreqs)
	t_r = xbands[0].t_r[nplasma]

	x = H_OVER_K * nu / (t_r)
	f = (2. * H * nu ** 3.) / (C ** 2. * (np.exp(x) - 1.))


	j =  f * xbands[0].w[nplasma]
